...
function getArtistByIndex(array, index) {
  return `the artist at index ${array[index], index} is ${array[index].name}`
} 
...
function get20s(array) {
  const artistYears = []; //new array to put results into.
  for (let i = 0; i < array.length; i++) { //loop over all of the indexes, starting at 0 and ending at the end of the objects.
  if (array[i].years >= '1900' && array[i].years <= '2000') { //if the indexes.years is greater than year1 and less than year2 then:
...
3. X The function should remove an artist from the array at the index
